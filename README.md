...
- **Audit log** of key actions (creation, updates, comments) recorded server-side.
...
python seed.py                  # wipes & populates demo data (users + ~18 tickets)
...
Tests cover ticket creation, SLA deadline/status math (on-time/at-risk/breached, including tickets resolved before vs. after their deadline), and access control — including an explicit test that an agent's tampered request can't reassign a ticket to themselves or another agent.
